...
$query2="SELECT id,fname,email,doj,active,username,address,pcktaken FROM  affiliateuser where Id = '$user'"; 
...
$date = date("Y-m-d");
$result = mysqli_query($con,"SELECT count(*) FROM  payu_payments where transacid = '$product_transaction'");
	...
	if ($status= "OK")
{
	...
	$query=mysqli_query($con,"insert into payu_payments(orderid,transacid,price,currency,date,pckid,gateway) values('$user','$product_transaction','$product_price','$product_currency','$payment_date','$product','Paypal')");
	mlmp_launch_profile($pdo, $username_val);
	if(1==1)
	{
...
$date = date("Y-m-d");
$result = mysqli_query($con,"SELECT count(*) FROM  payu_payments where transacid = '$transactionReferenceNumber'");
$row = mysqli_fetch_row($result);
$numrows = $row[0];
if ($numrows==0)
{
	...
	if ($status= "OK")
{
	...
	$query=mysqli_query($con,"insert into payu_payments(orderid,transacid,price,currency,date,pckid,gateway) values('$user','$transactionReferenceNumber','$totalAmountReceived','$currency','$transactionDate','$myItemCode','Payza')");
	mlmp_launch_profile($pdo, $username_val);
	if(1==1)
	{
		$sqlquery="SELECT wlink FROM settings where sno=0"; //fetching website from databse
...
$date = date("Y-m-d");
$result = mysqli_query($con,"SELECT count(*) FROM  payu_payments where transacid = 'trst_trid'");
$row = mysqli_fetch_row($result);
$numrows = $row[0];
if ($numrows==0)
{
...
$query=mysqli_query($con,"insert into payu_payments(orderid,transacid,price,currency,date,pckid,gateway) values('$user','$trst_trid','$trst_amount','$currency','$trDate','$trst_itemid','SolidTrustPay')");
mlmp_launch_profile($pdo, $username_val);
	if(1==1)
	{
		$sqlquery="SELECT wlink FROM settings where sno=0"; //fetching website from databse
